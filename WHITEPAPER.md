...
#### B. Sybil & Radio Channel Spam Mitigation
...
- **Solution**: Implement a lightweight **Radio-Level Proof-of-Work (RF-PoW)**. Before the gateway allocates CPU cycles to verify a ZK-SNARK, the sending node must solve a hash challenge:
...
| **Sybil Spam** | Gateway CPU | RF-Proof-of-Work (Hashcash) | Spamming requires massive compute; legitimate transmission is cheap. |
